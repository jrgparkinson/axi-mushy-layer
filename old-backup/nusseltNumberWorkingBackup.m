    ...
    for x_num_i = 1:numel(x_nums)
        ...
        for rm_i = 1:numel(Rms);
            ...
            for t_n = 1:t_num
                ...
                rhs_n = Rm * dT_dx;
                %{
for x_i = 2:numel(x_points)-1
                    for z_j = 2:numel(z_points)-1
                        rhs_n(x_i, z_j) = Rm * (T_n(x_i + 1, z_j) - T_n(x_i - 1, z_j))/(2*dx);
                    end
                end
                %}
                ...
                %3. Calculate psi^(n+1/2) from Poisson equation
                %{
for x_i = 2:numel(x_points)-1
                    for z_j = 2:numel(z_points)-1
                        rhs_nPlusHalf(x_i, z_j) = Rm * (T_nPlusHalf(x_i + 1, z_j) - T_nPlusHalf(x_i - 1, z_j))/(2*dx);
                    end
            end
                %}
